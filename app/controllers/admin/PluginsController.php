...
class Plugins {	
    ...
    public function directory(Request $request){
        ...
        if($request->q){
            $http = \Core\Http::url('https://cdn.gempixel.com.com/plugins/new.php')
                                ->with('X-Authorization', 'TOKEN '.config('purchasecode'))
                                ->body(clean($request->q))
                                ->post();
                                
        } elseif($request->category){
            $http = \Core\Http::url('https://cdn.gempixel.com.com/plugins/new.php?category='.$request->category)
                                ->with('X-Authorization', 'TOKEN '.config('purchasecode'))
                                ->post();
        } else {
            $http = \Core\Http::url('https://cdn.gempixel.com.com/plugins/new.php')
    ...
    public function install($request){
        ...
        $content = \Core\Http::url('https://cdn.gempixel.com.com/plugins/new.php?index='.clean($name))
                                ->with('X-Authorization', 'TOKEN '.config('purchasecode'))
                                ->post();

        if(!copy('https://cdn.gempixel.com.com/plugins/'.$name.'.zip', PLUGIN."/{$name}.zip")){
